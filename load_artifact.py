...
class Params:
    def __init__(self, args):
        ...
        self.project = check_not_none(args.project, "Project needs to be specified")
        self.branch = check_not_none(args.branch, "Branch must not be empty")
        self.path = check_not_none(args.path, "Artifact path must be specified")
        self.token = check_not_none(get_token(args.token), "Token must be specified")
...
def main():
    parser = argparse.ArgumentParser(description="Load an artifact from CircleCI")
    parser.add_argument("--user", "-u", dest="user", action="store", default="BjoernPetersen",
                        help="A CircleCI user/org")
    parser.add_argument("--project", "-p", dest="project", action="store", help="A CircleCI project name")
    parser.add_argument("--branch", "-b", dest="branch", action="store", default="master",
                        help="A branch to load from")
    parser.add_argument("--artifact", "-a", dest="path", action="store", help="The artifact path")
    parser.add_argument("--token", "-t", dest="token", action="store",
